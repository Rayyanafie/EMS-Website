...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['id'])) {
        ...
        $query = "SELECT * FROM tbl_regions WHERE id = '$nrp_upd'";
        //eksekusi query
        $result = mysqli_query(connection(), $query);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['delete'])) {
        ...
        $sql = "DELETE FROM tbl_regions WHERE id = '$nrp_upd'";
        $result = mysqli_query(connection(), $sql);
        if ($result) {
            echo "<script> alert('Region deleted successfully.');</script>
        <script> setTimeout(function() { window.location.href = 'tableregions.php'; }, 1000);</script>";
            exit;
        } else {
            echo "<script> alert('Failed to delete Region.');</script>
        <script> setTimeout(function() { window.location.href = 'tableregions.php'; }, 1000);</script>";
            exit;
        }
    } else {
